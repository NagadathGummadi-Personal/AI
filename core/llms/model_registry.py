...
class ModelFamily(str, Enum):
    ...
    AZURE_GPT_4_1 = "azure_gpt_4_1"
    AZURE_GPT_4_1_MINI = "azure_gpt_4_1_mini"
...
class ModelRegistry:
    ...
    def get_provider_models(self, provider: LLMProvider) -> List[str]:
        ...
        return [
            name for name, meta in self._models.items() if meta.provider == provider
        ]
    ...
    def validate_parameters_for_model(
        ...
        if (
            ...
            and not metadata.supports_frequency_penalty
        ):
            issues.append(
                f"Model {model_name} does not support frequency_penalty parameter"
            )

        if "presence_penalty" in parameters and not metadata.supports_presence_penalty:
            issues.append(
                f"Model {model_name} does not support presence_penalty parameter"
            )
